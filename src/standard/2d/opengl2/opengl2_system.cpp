...
const std::unordered_map<JamJar::Standard::_2D::OpenGLShaderType, GLenum> SHADER_TYPES = {
    {JamJar::Standard::_2D::OpenGLShaderType::VERTEX, GL_VERTEX_SHADER},
    {JamJar::Standard::_2D::OpenGLShaderType::FRAGMENT, GL_FRAGMENT_SHADER}};

JamJar::Standard::_2D::OpenGLSystem::OpenGLSystem(MessageBus *messageBus, SDL_Window *window)
    : RenderSystem(messageBus, OpenGLSystem::evaluator), window(window),
    ...
    this->messageBus->Subscribe(this, JamJar::Standard::_2D::OpenGLSystem::MESSAGE_LOAD_SHADER);
...
bool JamJar::Standard::_2D::OpenGLSystem::evaluator(Entity *entity, std::vector<JamJar::Component *> components) {
...
void JamJar::Standard::_2D::OpenGLSystem::OnMessage(JamJar::Message *message) {
    ...
    case JamJar::Standard::_2D::OpenGLSystem::MESSAGE_LOAD_SHADER: {
        auto *loadMessage =
            static_cast<JamJar::MessagePayload<std::unique_ptr<JamJar::Standard::_2D::OpenGLShader>> *>(message);
        this->loadShader(std::move(loadMessage->payload));
    }
    }
}

void JamJar::Standard::_2D::OpenGLSystem::loadTexture(FileTextureResponse *response) {
...
GLuint JamJar::Standard::_2D::OpenGLSystem::createTexture(const int width, const int height,
...
void JamJar::Standard::_2D::OpenGLSystem::loadShader(std::unique_ptr<OpenGLShader> shader) {
    ...
    auto loadedShader =
        std::make_unique<OpenGLLoadedShader>(OpenGLLoadedShader({.definition = std::move(shader), .shader = glShader}));

    this->m_shaders.insert({loadedShader->definition->name, std::move(loadedShader)});
}

GLuint JamJar::Standard::_2D::OpenGLSystem::getProgram(std::string identifier, GLuint fragment, GLuint vertex) {
...
void JamJar::Standard::_2D::OpenGLSystem::render(float deltaTime) {
    for (auto &entity : this->entities) {
        ...
        for (auto renderable : cameraRenderables) {
            ...
            auto context =
                std::make_unique<JamJar::Standard::_2D::OpenGLShaderContext>(JamJar::Standard::_2D::OpenGLShaderContext(
